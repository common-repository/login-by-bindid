			...
			<?php foreach ( $logs as $log ) { ?>
				<tr>
					<td class="col-details">
						<div>
							<label><?php echo esc_html(__( 'Type', 'bindid' )); ?>: </label>
							<?php echo esc_html($log['type']); ?>
						</div>
						<div>
							<label><?php echo esc_html(__( 'Date', 'bindid' )); ?>: </label>
							<?php echo esc_html(gmdate( 'Y-m-d H:i:s', $log['time'] )); ?>
						</div>
						<div>
							<label><?php echo esc_html(__( 'User', 'bindid' )); ?>: </label>
							<?php echo ( get_userdata( $log['user_ID'] ) ) ? get_userdata( $log['user_ID'] )->user_login : '0'; ?>
						</div>
						<div>
							<label><?php echo esc_html(__( 'URI ', 'bindid' )); ?>: </label>
							<?php echo esc_html($log['uri']); ?>
						</div>
					</td>

					<td class="col-data"><pre><?php echo esc_html(var_dump( $log['data'] )); ?></pre></td>
				</tr>
			<?php } ?>
			</tbody>
		</table>
